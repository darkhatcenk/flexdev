...
class SyncBunny extends Command
{
    ...
    private function syncReleasesToGitHubRepo(): bool
    {
        ...
        try {
            $response = Http::timeout(30)
                ->get('https://api.github.com/repos/cloudflex/flexdev/releases', [
            ...
            $timestamp = time();
            $tmpDir = sys_get_temp_dir().'/flexdev-cdn-'.$timestamp;
            $branchName = 'update-releases-'.$timestamp;

            // Clone the repository
            $this->info('Cloning flexdev-cdn repository...');
            exec('gh repo clone cloudflex/flexdev-cdn '.escapeshellarg($tmpDir).' 2>&1', $output, $returnCode);
            if ($returnCode !== 0) {
            ...
            $prCommand = 'gh pr create --repo cloudflex/flexdev-cdn --title '.escapeshellarg($prTitle).' --body '.escapeshellarg($prBody).' --base main --head '.escapeshellarg($branchName).' 2>&1';
    ...
    public function handle()
    {
        ...
        $bunny_cdn = 'https://cdn.cloudflex.io';
        $bunny_cdn_path = 'flexdev';
        ...
        try {
            if ($nightly) {
                $bunny_cdn_path = 'flexdev-nightly';
            ...
            if ($only_template) {
                ...
                $actual_version = data_get($json, 'flexdev.v4.version');
