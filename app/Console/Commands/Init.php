...
class Init extends Command
{
    ...
    public function handle()
    {
        ...
        $this->restoreFlexDEVDbBackup();
        $this->updateUserEmails();
        //
        $this->updateTraefikLabels();
        $this->cleanupUnusedNetworkFromFlexDEVProxy();
        ...
        try {
            ...
            echo "Continuing with initialization - cleanup errors will not prevent FlexDEV from starting\n";
        }
        try {
        ...
        try {
            $updatedTaskCount = ScheduledTaskExecution::where('status', 'running')->update([
                'status' => 'failed',
                'message' => 'Marked as failed during FlexDEV startup - job was interrupted',
        ...
        if (! is_null(config('constants.flexdev.autoupdate', null))) {
            if (config('constants.flexdev.autoupdate') == true) {
    ...
    private function cleanupUnusedNetworkFromFlexDEVProxy()
    {
        foreach ($this->servers as $server) {
            ...
            try {
                ...
                foreach ($removeNetworks as $network) {
                    ...
                    if (empty($out)) {
                        $commands->push("docker network disconnect $network flexdev-proxy >/dev/null 2>&1 || true");
                        $commands->push("docker network rm $network >/dev/null 2>&1 || true");
                    } else {
                        $data = collect(json_decode($out, true));
                        if ($data->count() === 1) {
                            // If only flexdev-proxy itself is connected to that network (it should not be possible, but who knows)
                            $isFlexDEVProxyItself = data_get($data->first(), 'Name') === 'flexdev-proxy';
                            if ($isFlexDEVProxyItself) {
                                $commands->push("docker network disconnect $network flexdev-proxy >/dev/null 2>&1 || true");
                                $commands->push("docker network rm $network >/dev/null 2>&1 || true");
                            }
                        }
                ...
                echo "Error in cleaning up unused networks from flexdev proxy: {$e->getMessage()}\n";
            }
        }
    }

    private function restoreFlexDEVDbBackup()
    {
        if (version_compare('4.0.0-beta.179', config('constants.flexdev.version'), '<=')) {
            try {
                ...
                echo "Error in restoring flexdev db backup: {$e->getMessage()}\n";
            }
        }
    }
    ...
    {
        $id = config('app.id');
        $version = config('constants.flexdev.version');
        try {
            Http::get("https://undead.flexdev.io/v4/alive?appId=$id&version=$version");
        } catch (\Throwable $e) {
    ...
    {
        if (version_compare('4.0.0-beta.298', config('constants.flexdev.version'), '<=')) {
