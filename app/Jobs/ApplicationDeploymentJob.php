...
class ApplicationDeploymentJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    private ?string $flexdev_variables = null;
            ...
            if (! is_null($allContainers)) {
                ...
                if (count($allContainers) > 0) {
                    ...
                    foreach ($allContainers as $container) {
                        ...
                        if ($containerName === 'flexdev-proxy') {
                            continue;
                        }
    ...
    private function deploy_docker_compose_buildpack()
    {
        ...
        if ($this->docker_compose_custom_build_command) {
            ...
            $command = "{$this->flexdev_variables} docker compose";
            // Prepend DOCKER_BUILDKIT=1 if BuildKit is supported
            if ($this->dockerBuildkitSupported) {
        ...
        if ($this->server->isSwarm()) {
            ...
            $this->execute_remote_command([
                ...
                "docker network connect {$networkId} flexdev-proxy >/dev/null 2>&1 || true",
                'hidden' => true,
                'ignore_errors' => true,
            ]);
        }
    ...
    private function generate_runtime_environment_variables()
    {
        ...
        $flexdev_envs = $this->generate_flexdev_env_variables();
        $flexdev_envs->each(function ($item, $key) use ($envs) {
            $envs->push($key.'='.$item);
        });
        if ($this->pull_request_id === 0) {
            ...
            if ($this->build_pack === 'dockercompose') {
                ...
                foreach ($domains as $forServiceName => $domain) {
                    ...
                    if (filled($parsedDomain)) {
                        ...
                        $flexdevUrl = Url::fromString($parsedDomain);
                        $flexdevScheme = $flexdevUrl->getScheme();
                        $flexdevFqdn = $flexdevUrl->getHost();
                        $flexdevUrl = $flexdevUrl->withScheme($flexdevScheme)->withHost($flexdevFqdn)->withPort(null);
                        $envs->push('SERVICE_URL_'.str($forServiceName)->upper().'='.$flexdevUrl->__toString());
                        $envs->push('SERVICE_FQDN_'.str($forServiceName)->upper().'='.$flexdevFqdn);
                    }
                }
            ...
            if ($this->pull_request_id === 0) {
                ...
                foreach ($domains as $forServiceName => $domain) {
                    ...
                    if (filled($parsedDomain)) {
                        ...
                        $flexdevFqdn = $flexdevUrl->getHost();
                        $flexdevUrl = $flexdevUrl->withScheme($flexdevScheme)->withHost($flexdevFqdn)->withPort(null);
                        $envs->push('SERVICE_URL_'.str($forServiceName)->upper().'='.escapeBashEnvValue($flexdevUrl->__toString()));
                        $envs->push('SERVICE_FQDN_'.str($forServiceName)->upper().'='.escapeBashEnvValue($flexdevFqdn));
                    }
                }
            } else {
    ...
    private function set_flexdev_variables()
    {
        $this->flexdev_variables = "SOURCE_COMMIT={$this->commit} ";
        if ($this->pull_request_id === 0) {
        ...
        if (isset($fqdn)) {
            ...
            if ((int) $this->application->compose_parsing_version >= 3) {
                $this->flexdev_variables .= "FLEXDEV_URL={$url} ";
                $this->flexdev_variables .= "FLEXDEV_FQDN={$fqdn} ";
            } else {
                $this->flexdev_variables .= "FLEXDEV_URL={$fqdn} ";
                $this->flexdev_variables .= "FLEXDEV_FQDN={$url} ";
            }
        }
        if (isset($this->application->git_branch)) {
            $this->flexdev_variables .= "FLEXDEV_BRANCH={$this->application->git_branch} ";
        }
        $this->flexdev_variables .= "FLEXDEV_RESOURCE_UUID={$this->application->uuid} ";
        $this->flexdev_variables .= "FLEXDEV_CONTAINER_NAME={$this->container_name} ";
    }
    ...
    private function generate_nixpacks_env_variables()
    {
        ...
        // Add FLEXDEV_* environment variables to Nixpacks build context
        $flexdev_envs = $this->generate_flexdev_env_variables();
        $flexdev_envs->each(function ($value, $key) {
    ...
    private function generate_flexdev_env_variables(): Collection
    {
        $flexdev_envs = collect([]);
        $local_branch = $this->branch;
        if ($this->pull_request_id !== 0) {
            // Add SOURCE_COMMIT if not exists
            if ($this->application->environment_variables_preview->where('key', 'SOURCE_COMMIT')->isEmpty()) {
                if (! is_null($this->commit)) {
                    $flexdev_envs->put('SOURCE_COMMIT', $this->commit);
                } else {
                    $flexdev_envs->put('SOURCE_COMMIT', 'unknown');
                }
            }
            if ($this->application->environment_variables_preview->where('key', 'FLEXDEV_FQDN')->isEmpty()) {
                if ((int) $this->application->compose_parsing_version >= 3) {
                    $flexdev_envs->put('FLEXDEV_URL', $this->preview->fqdn);
                } else {
                    $flexdev_envs->put('FLEXDEV_FQDN', $this->preview->fqdn);
                }
            }
            if ($this->application->environment_variables_preview->where('key', 'FLEXDEV_URL')->isEmpty()) {
                $url = str($this->preview->fqdn)->replace('http://', '')->replace('https://', '');
                if ((int) $this->application->compose_parsing_version >= 3) {
                    $flexdev_envs->put('FLEXDEV_FQDN', $url);
                } else {
                    $flexdev_envs->put('FLEXDEV_URL', $url);
                }
            }
            if ($this->application->build_pack !== 'dockercompose' || $this->application->compose_parsing_version === '1' || $this->application->compose_parsing_version === '2') {
                if ($this->application->environment_variables_preview->where('key', 'FLEXDEV_BRANCH')->isEmpty()) {
                    $flexdev_envs->put('FLEXDEV_BRANCH', $local_branch);
                }
                if ($this->application->environment_variables_preview->where('key', 'FLEXDEV_RESOURCE_UUID')->isEmpty()) {
                    $flexdev_envs->put('FLEXDEV_RESOURCE_UUID', $this->application->uuid);
                }
                if ($this->application->environment_variables_preview->where('key', 'FLEXDEV_CONTAINER_NAME')->isEmpty()) {
                    $flexdev_envs->put('FLEXDEV_CONTAINER_NAME', $this->container_name);
                }
            }

            add_flexdev_default_environment_variables($this->application, $flexdev_envs, $this->application->environment_variables_preview);
            ...
            if ($this->application->environment_variables->where('key', 'FLEXDEV_FQDN')->isEmpty()) {
                if ((int) $this->application->compose_parsing_version >= 3) {
                    $flexdev_envs->put('FLEXDEV_URL', $this->application->fqdn);
                } else {
                    $flexdev_envs->put('FLEXDEV_FQDN', $this->application->fqdn);
                }
            }
            if ($this->application->environment_variables->where('key', 'FLEXDEV_URL')->isEmpty()) {
            ...
            if ($this->application->build_pack !== 'dockercompose' || $this->application->compose_parsing_version === '1' || $this->application->compose_parsing_version === '2') {
                if ($this->application->environment_variables->where('key', 'FLEXDEV_BRANCH')->isEmpty()) {
                    $flexdev_envs->put('FLEXDEV_BRANCH', $local_branch);
                }
                if ($this->application->environment_variables->where('key', 'FLEXDEV_RESOURCE_UUID')->isEmpty()) {
                    $flexdev_envs->put('FLEXDEV_RESOURCE_UUID', $this->application->uuid);
                }
                if ($this->application->environment_variables->where('key', 'FLEXDEV_CONTAINER_NAME')->isEmpty()) {
                    $flexdev_envs->put('FLEXDEV_CONTAINER_NAME', $this->container_name);
                }
            }

            add_flexdev_default_environment_variables($this->application, $flexdev_envs, $this->application->environment_variables);
    ...
    private function generate_compose_file()
    {
        ...
        if (data_get($this->application, 'custom_labels')) {
            ...
            $labels = $labels->filter(function ($value, $key) {
                return ! Str::startsWith($value, 'flexdev.');
            });
        ...
        // Only add FlexDEV healthcheck if no custom HEALTHCHECK found in Dockerfile
    ...
    private function build_static_image()
    {
        ...
        LABEL flexdev.deploymentId={$this->deployment_uuid}
        COPY . .
     ...
     * This enables shell interpolation of variables (e.g., APP_URL=$FLEXDEV_URL)
     *
    ...
    {
        // Add FlexDEV related variables to the build args/secrets
        if ($this->dockerBuildkitSupported) {
            // FlexDEV variables are already included in the secrets from generate_build_env_variables
            // build_secrets is already a string at this point
        } else {
            // Traditional build args approach - generate FLEXDEV_ variables locally
            // Generate FLEXDEV_ variables locally for build args
            $flexdev_envs = $this->generate_flexdev_env_variables();
            $flexdev_envs->each(function ($value, $key) {
...
COPY ./nginx.conf /etc/nginx/conf.d/default.conf");
        ...
        try {
            ...
            if ($this->newVersionIsHealthy || $force) {
                ...
                if ($this->application->dockerfile || $this->application->build_pack === 'dockerfile' || $this->application->build_pack === 'dockerimage') {
                    $this->application_deployment_queue->addLogEntry('----------------------------------------');
                    $this->application_deployment_queue->addLogEntry("WARNING: Dockerfile or Docker Image based deployment detected. The healthcheck needs a curl or wget command to check the health of the application. Please make sure that it is available in the image or turn off healthcheck on FlexDEV's UI.");
                    $this->application_deployment_queue->addLogEntry('----------------------------------------');
                }
        ...
        try {
            ...
            if ($this->application->build_pack === 'dockerimage') {
                ...
                $this->execute_remote_command(
                    ...
                    [executeInDocker($this->deployment_uuid, "{$this->flexdev_variables} docker compose --project-name {$this->application->uuid} --project-directory {$this->workdir} up --build -d"), 'hidden' => true],
                );
            } else {
                if ($this->use_build_server) {
                    $this->execute_remote_command(
                        ["{$this->flexdev_variables} docker compose --project-name {$this->application->uuid} --project-directory {$this->configuration_dir} -f {$this->configuration_dir}{$this->docker_compose_location} up --pull always --build -d", 'hidden' => true],
                    );
                } else {
                    $this->execute_remote_command(
                        [executeInDocker($this->deployment_uuid, "{$this->flexdev_variables} docker compose --project-name {$this->application->uuid} --project-directory {$this->workdir} -f {$this->workdir}{$this->docker_compose_location} up --build -d"), 'hidden' => true],
                    );
                }
            }
            $this->application_deployment_queue->addLogEntry('New container started.');
    ...
    private function generate_build_env_variables()
    {
        ...
        if ($this->dockerBuildkitSupported && $this->application->settings->use_build_secrets) {
            ...
            if ($secrets_hash) {
                $this->build_args->push("--build-arg FLEXDEV_BUILD_SECRETS_HASH={$secrets_hash}");
            }
        }
    }
    ...
    private function generate_docker_env_flags_for_secrets()
    {
        ...
        // This populates $this->env_args with both user-defined and FLEXDEV_* variables
        ...
        $env_flags .= " -e FLEXDEV_BUILD_SECRETS_HASH={$secrets_hash}";
    ...
    private function generate_build_secrets(Collection $variables)
    {
        ...
        $this->build_secrets .= ' --secret id=FLEXDEV_BUILD_SECRETS_HASH,env=FLEXDEV_BUILD_SECRETS_HASH';
    }
    ...
    private function add_build_env_variables_to_dockerfile()
    {
        if ($this->dockerBuildkitSupported) {
            ...
            if ($this->pull_request_id === 0) {
                ...
                // Add FlexDEV variables as ARGs
                if ($this->flexdev_variables) {
                    $flexdev_vars = collect(explode(' ', trim($this->flexdev_variables)))
                        ->filter()
                        ->map(function ($var) {
                            return "ARG {$var}";
                        });
                    $argsToInsert = $argsToInsert->merge($flexdev_vars);
                }
            } else {
            ...
            if ($argsToInsert->isNotEmpty()) {
                ...
                $argsToInsert->push("ARG FLEXDEV_BUILD_SECRETS_HASH={$secrets_hash}");
            }
    ...
    private function modify_dockerfile_for_secrets($dockerfile_path)
    {
        ...
        $mountStrings .= ' --mount=type=secret,id=FLEXDEV_BUILD_SECRETS_HASH,env=FLEXDEV_BUILD_SECRETS_HASH';
