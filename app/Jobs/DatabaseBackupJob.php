...
class DatabaseBackupJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            if ($this->database->name === 'flexdev-db') {
                $databasesToBackup = ['flexdev'];
                $this->directory_name = $this->container_name = 'flexdev-db';
                $ip = Str::slug($this->server->ip);
                $this->backup_dir = backup_dir().'/flexdev'."/flexdev-db-$ip";
            }
    ...
    private function upload_to_s3(): void
    {
        try {
            ...
            if (isDev()) {
                if ($this->database->name === 'flexdev-db') {
                    $backup_location_from = '/var/lib/docker/volumes/flexdev_dev_backups_data/_data/flexdev/flexdev-db-'.$this->server->ip.$this->backup_file;
                    ...
                    $backup_location_from = '/var/lib/docker/volumes/flexdev_dev_backups_data/_data/databases/'.str($this->team->name)->slug().'-'.$this->team->id.'/'.$this->directory_name.$this->backup_file;
                    $commands[] = "docker run -d --network {$network} --name backup-of-{$this->backup_log_uuid} --rm -v $backup_location_from:$this->backup_location:ro {$fullImageName}";
                }
            } else {
