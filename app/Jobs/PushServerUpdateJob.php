...
class PushServerUpdateJob implements ShouldBeEncrypted, ShouldQueue, Silenced
{
    ...
    public function handle()
    {
        ...
        foreach ($this->containers as $container) {
            ...
            $flexdev_managed = $labels->has('flexdev.managed');
            ...
            if ($name === 'flexdev-log-drain' && $this->isRunning($containerStatus)) {
                $this->foundLogDrainContainer = true;
            }
            if ($labels->has('flexdev.applicationId')) {
                $applicationId = $labels->get('flexdev.applicationId');
                $pullRequestId = $labels->get('flexdev.pullRequestId', '0');
                try {
                    if ($pullRequestId === '0') {
            ...
            } elseif ($labels->has('flexdev.serviceId')) {
                $serviceId = $labels->get('flexdev.serviceId');
                $subType = $labels->get('flexdev.service.subType');
                $subId = $labels->get('flexdev.service.subId');
                ...
                $type = $labels->get('flexdev.type');
                if ($name === 'flexdev-proxy' && $this->isRunning($containerStatus)) {
            ...
            if ($this->foundProxy === false) {
                try {
                    if (CheckProxy::run($this->server)) {
                        ...
                        $this->server->team?->notify(new ContainerRestarted('flexdev-proxy', $this->server));
                    }
                } catch (\Throwable $e) {
                }
            } else {
