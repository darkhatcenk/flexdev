...
class CleanupHelperContainersJob implements ShouldBeEncrypted, ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $containers = instant_remote_process_with_timeout(['docker container ps --format \'{{json .}}\' | jq -s \'map(select(.Image | contains("'.config('constants.flexdev.registry_url').'/cloudflex/flexdev-helper")))\''], $this->server, false);
