...
class ByHetzner extends Component
{
    ...
    private function createHetznerServer(string $token): array
    {
        ...
        // Prepare SSH keys array: FlexDEV key + user-selected Hetzner keys
        $sshKeys = array_merge(
            [$sshKeyId], // FlexDEV key (always included)
            $this->selectedHetznerSshKeyIds // User-selected Hetzner keys
        );

        // Remove duplicates in case the FlexDEV key was also selected
    ...
    public function submit()
    {
        ...
        try {
            ...
            // Create server in FlexDEV database
