            ...
            if ($previewKey !== false && isset($this->previewFqdns[$previewKey])) {
                ...
                if (! empty($fqdn)) {
                    ...
                    if (! validateDNSEntry($fqdn, $this->application->destination->server)) {
                        $this->dispatch('error', 'Validating DNS failed.', "Make sure you have added the DNS records correctly.<br><br>$fqdn->{$this->application->destination->server->ip}<br><br>Check this <a target='_blank' class='underline dark:text-white' href='https://flexdev.io/docs/knowledge-base/dns-configuration'>documentation</a> for further help.");
                        $success = false;
                    }
