...
class ComplexStatusCheck
{
    ...
    public function handle(Application $application)
    {
        ...
        foreach ($servers as $server) {
            ...
            $containers = instant_remote_process(["docker container inspect $(docker container ls -q --filter 'label=flexdev.applicationId={$application->id}' --filter 'label=flexdev.pullRequestId=0') --format '{{json .}}'"], $server, false);
