    ...
    public function handle(Server $server, ?Collection $containers = null, ?Collection $containerReplicates = null)
    {
        ...
        foreach ($this->containers as $container) {
            if ($this->server->isSwarm()) {
                ...
                $uuid = data_get($labels, 'flexdev.name');
            } else {
            ...
            $applicationId = data_get($labels, 'flexdev.applicationId');
            if ($applicationId) {
                $pullRequestId = data_get($labels, 'flexdev.pullRequestId');
                if ($pullRequestId) {
                ...
                $type = data_get($labels, 'flexdev.type');

                if ($uuid) {
                    if ($type === 'service') {
                        $database_id = data_get($labels, 'flexdev.service.subId');
                        if ($database_id) {
                                ...
                                if ($uuid) {
                                    ...
                                    if ($isPublic) {
                                        $foundTcpProxy = $this->containers->filter(function ($value, $key) use ($uuid) {
                                            if ($this->server->isSwarm()) {
                                                return data_get($value, 'Spec.Name') === "flexdev-proxy_$uuid";
                                            } else {
                ...
                if (data_get($container, 'Name') === '/flexdev-db') {
                    $foundDatabases[] = 0;
                }
            }
            $serviceLabelId = data_get($labels, 'flexdev.serviceId');
            if ($serviceLabelId) {
                $subType = data_get($labels, 'flexdev.service.subType');
                $subId = data_get($labels, 'flexdev.service.subId');
