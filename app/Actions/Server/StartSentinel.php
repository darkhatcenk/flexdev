...
class StartSentinel
{
    ...
    public function handle(Server $server, bool $restart = false, ?string $latestVersion = null, ?string $customImage = null)
    {
        ...
        $mountDir = '/data/flexdev/sentinel';
        $image = config('constants.flexdev.registry_url').'/cloudflex/sentinel:'.$version;
        if (! $endpoint) {
        ...
        if (isDev()) {
            ...
            $mountDir = '/var/lib/docker/volumes/flexdev_dev_flexdev_data/_data/sentinel';
        }
        ...
        $dockerCommand = "docker run -d $dockerEnvironments --name flexdev-sentinel -v /var/run/docker.sock:/var/run/docker.sock -v $mountDir:/app/db --pid host --health-cmd \"curl --fail http://127.0.0.1:8888/api/health || exit 1\" --health-interval 10s --health-retries 3 --add-host=host.docker.internal:host-gateway --label $dockerLabels $image";

        instant_remote_process([
            'docker rm -f flexdev-sentinel || true',
            "mkdir -p $mountDir",
            $dockerCommand,
