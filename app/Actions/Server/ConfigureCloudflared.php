    ...
    public function handle(Server $server, string $cloudflare_token, string $ssh_domain): Activity
    {
        try {
            $config = [
                'services' => [
                    'flexdev-cloudflared' => [
                        'container_name' => 'flexdev-cloudflared',
            ...
            $commands = collect([
                ...
                'docker rm -f flexdev-cloudflared || true',
                'echo Starting new Cloudflare Tunnel container.',
                'docker compose up --wait --wait-timeout 15 --remove-orphans || docker logs flexdev-cloudflared',
            ]);
