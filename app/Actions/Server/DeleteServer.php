...
class DeleteServer
{
    ...
    public function handle(int $serverId, bool $deleteFromHetzner = false, ?int $hetznerServerId = null, ?int $cloudProviderTokenId = null, ?int $teamId = null)
    {
        ...
        // Delete from Hetzner even if server is already gone from FlexDEV
        ...
        ray($server ? 'Deleting server from FlexDEV' : 'Server already deleted from FlexDEV, skipping FlexDEV deletion');

        // If server is already deleted from FlexDEV, skip this part
        if (! $server) {
            return; // Server already force deleted from FlexDEV
        }

        ray('force deleting server from FlexDEV', ['server_id' => $server->id]);

        try {
            $server->forceDelete();
        } catch (\Throwable $e) {
            ray('Failed to force delete server from FlexDEV', [
                'error' => $e->getMessage(),
                'server_id' => $server->id,
            ]);
            logger()->error('Failed to force delete server from FlexDEV', [
                'error' => $e->getMessage(),
                'server_id' => $server->id,
            ]);
        }
    ...
    private function deleteFromHetznerById(int $hetznerServerId, ?int $cloudProviderTokenId, int $teamId): void
    {
        try {
            ...
            // Log the error but don't prevent the server from being deleted from FlexDEV
