...
class CheckProxy
{
    ...
    public function handle(Server $server, $fromUI = false): bool
    {
        ...
        $proxyContainerName = $server->isSwarm() ? 'flexdev-proxy_traefik' : 'flexdev-proxy';
        ...
        if ($server->isSwarm()) {
            ...
            foreach ($conflicts as $port => $conflict) {
                if ($conflict) {
                    if ($fromUI) {
                        throw new \Exception("Port $port is in use.<br>You must stop the process using this port.<br><br>Docs: <a target='_blank' class='dark:text-white hover:underline' href='https://flexdev.io/docs'>https://flexdev.io/docs</a><br>Discord: <a target='_blank' class='dark:text-white hover:underline' href='https://flexdev.io/discord'>https://flexdev.io/discord</a>");
                    } else {
                        return false;
                    }
                }
    ...
    private function buildPortCheckCommands(Server $server, string $port, string $proxyContainerName): array
    {
        ...
        $portCheckScript = "
            ...
            if command -v ss >/dev/null 2>&1; then
                ...
                if [ \$count -le 2 ] && (echo \"\$ss_output\" | grep -q 'docker\\|flexdev'); then
                    echo 'port_free';
                    exit 0;
                fi;
            ...
            if command -v netstat >/dev/null 2>&1; then
                ...
                if [ \$count -le 2 ] && (echo \"\$netstat_output\" | grep -q 'docker\\|flexdev'); then
                    echo 'port_free';
                    exit 0;
                fi;
    ...
    private function isPortConflict(Server $server, string $port, string $proxyContainerName): bool
    {
        ...
        foreach ($commandSets as $set) {
            try {
                ...
                // Try to detect if this is our flexdev-proxy
