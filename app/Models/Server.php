...
class Server extends BaseModel
{
    ...
    protected $appends = ['is_flexdev_host'];
    ...
    protected function isFlexDEVHost(): Attribute
    {
    ...
    {
        $banner =
            "# This file is generated by FlexDEV, do not edit it manually.\n".
        ...
        if (isDev()) {
            if ($proxy_type === ProxyTypes::CADDY->value) {
                $dynamic_conf_path = '/data/flexdev/proxy/caddy/dynamic';
            }
        }
   ...
   redir $redirect_url
    ...
    public function setupDynamicProxyConfiguration()
    {
        ...
        if ($this->proxyType() === ProxyTypes::TRAEFIK->value) {
            $file = "$dynamic_config_path/flexdev.yaml";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                        ...
                        'services' => [
                            ...
                            'flexdev-realtime' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://flexdev-realtime:6001',
                                        ],
                                    ],
                                ],
                            ],
                            'flexdev-terminal' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://flexdev-realtime:6002',
                                        ],
                                    ],
                                ],
                            ],
                ...
                if ($schema === 'https') {
                    $traefik_dynamic_conf['http']['routers']['flexdev-http']['middlewares'] = [
                        0 => 'redirect-to-https',
                    ];

                    $traefik_dynamic_conf['http']['routers']['flexdev-https'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'flexdev',
                    ...
                    $traefik_dynamic_conf['http']['routers']['flexdev-realtime-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'flexdev-realtime',
                    ...
                    $traefik_dynamic_conf['http']['routers']['flexdev-terminal-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'flexdev-terminal',
                ...
                $yaml =
                    "# This file is automatically generated by FlexDEV.\n".
            ...
            $file = "$dynamic_config_path/flexdev.caddy";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                instant_remote_process([
                    "rm -f $file",
                ], $this);
...
$schema://$host {
    handle /app/* {
        reverse_proxy flexdev-realtime:6001
    }
    handle /terminal/ws {
        reverse_proxy flexdev-realtime:6002
    }
    reverse_proxy flexdev:8080
}";
    ...
    public function reloadCaddy()
    {
        return instant_remote_process([
            'docker exec flexdev-proxy caddy reload --config /config/caddy/Caddyfile.autosave',
        ], $this);
    }

    public function proxyPath()
    {
        $base_path = config('constants.flexdev.base_config_path');
    ...
    public function getCpuMetrics(int $mins = 5)
    {
        if ($this->isMetricsEnabled()) {
            ...
            $cpu = instant_remote_process(["docker exec flexdev-sentinel sh -c 'curl -H \"Authorization: Bearer {$this->settings->sentinel_token}\" http://localhost:8888/api/cpu/history?from=$from'"], $this, false);
    ...
    public function getMemoryMetrics(int $mins = 5)
    {
        if ($this->isMetricsEnabled()) {
            $from = now()->subMinutes($mins)->toIso8601ZuluString();
            $memory = instant_remote_process(["docker exec flexdev-sentinel sh -c 'curl -H \"Authorization: Bearer {$this->settings->sentinel_token}\" http://localhost:8888/api/memory/history?from=$from'"], $this, false);
    ...
    public function loadUnmanagedContainers(): Collection
    {
        if ($this->isFunctional()) {
            ...
            $containers = $containers->map(function ($container) {
                ...
                if (! str($labels)->contains('flexdev.managed')) {
                    return $container;
                }
    ...
    public function databases()
    {
        return $this->destinations()->map(function ($standaloneDocker) {
            ...
            return data_get($item, 'name') !== 'flexdev-db';
        });
    }
    ...
    public function validateDockerEngine($throwError = false)
    {
        ...
        $this->settings->save();
        $this->validateFlexDEVNetwork(isSwarm: false, isBuildServer: $this->settings->is_build_server);
    ...
    public function validateDockerSwarm()
    {
        ...
        $this->settings->is_usable = true;
        $this->settings->save();
        $this->validateFlexDEVNetwork(isSwarm: true);
    ...
    public function validateFlexDEVNetwork($isSwarm = false, $isBuildServer = false)
    {
        if ($isBuildServer) {
            return;
        }
        if ($isSwarm) {
            return instant_remote_process(['docker network create --attachable --driver overlay flexdev-overlay >/dev/null 2>&1 || true'], $this, false);
        } else {
            return instant_remote_process(['docker network create flexdev --attachable >/dev/null 2>&1 || true'], $this, false);
        }
    }
    ...
    public function generateCaCertificate()
    {
        try {
            ...
            SslHelper::generateSslCertificate(
                commonName: 'FlexDEV CA Certificate',
                serverId: $this->id,
            ...
            if ($caCertificate) {
                ...
                $caCertPath = config('constants.flexdev.base_config_path').'/ssl/';

                $commands = collect([
                    "mkdir -p $caCertPath",
                    "chown -R 9999:root $caCertPath",
                    "chmod -R 700 $caCertPath",
                    "rm -rf $caCertPath/flexdev-ca.crt",
                    "echo '{$certificateContent}' > $caCertPath/flexdev-ca.crt",
                    "chmod 644 $caCertPath/flexdev-ca.crt",
                ]);
