...
#[OA\Schema(
    ...
    properties: [
        ...
        'compose_parsing_version' => ['type' => 'string', 'description' => 'How FlexDEV parse the compose file.'],
...
class Application extends BaseModel
{
    ...
    public function deleteConnectedNetworks()
    {
        $uuid = $this->uuid;
        $server = data_get($this, 'destination.server');
        instant_remote_process(["docker network disconnect {$uuid} flexdev-proxy"], $server, false);
    ...
    public function generateGitImportCommands(string $deployment_uuid, int $pull_request_id = 0, ?string $git_type = null, bool $exec_in_docker = true, bool $only_checkout = false, ?string $custom_base_dir = null, ?string $commit = null)
    {
        ...
        if ($pull_request_id !== 0) {
            $pr_branch_name = "pr-{$pull_request_id}-flexdev";
        }
        if ($this->deploymentType() === 'source') {
            $source_html_url = data_get($this, 'source.html_url');
            $url = parse_url(filter_var($source_html_url, FILTER_SANITIZE_URL));
    ...
    public function oldRawParser()
    {
        ...
        $services = collect($services)->map(function ($service) use ($commands) {
            ...
            if (! $labels->contains('flexdev.managed')) {
                $labels->push('flexdev.managed=true');
            }
            if (! $labels->contains('flexdev.applicationId')) {
                $labels->push('flexdev.applicationId='.$this->id);
            }
            if (! $labels->contains('flexdev.type')) {
                $labels->push('flexdev.type=application');
            }
    ...
    public function parseContainerLabels(?ApplicationPreview $preview = null)
    {
        ...
        if (mb_detect_encoding($customLabels, 'ASCII', true) === false) {
            $customLabels = str(implode('|flexdev|', generateLabelsApplication($this, $preview)))->replace('|flexdev|', "\n");
        }
        $this->custom_labels = base64_encode($customLabels);
        $this->save();
    ...
    public function getCpuMetrics(int $mins = 5)
    {
        ...
        if ($server->isMetricsEnabled()) {
            ...
            $metrics = instant_remote_process(["docker exec flexdev-sentinel sh -c 'curl -H \"Authorization: Bearer {$server->settings->sentinel_token}\" http://localhost:8888/api/container/{$container_name}/cpu/history?from=$from'"], $server, false);
    ...
    public function getMemoryMetrics(int $mins = 5)
    {
        ...
        if ($server->isMetricsEnabled()) {
            $from = now()->subMinutes($mins)->toIso8601ZuluString();
            $metrics = instant_remote_process(["docker exec flexdev-sentinel sh -c 'curl -H \"Authorization: Bearer {$server->settings->sentinel_token}\" http://localhost:8888/api/container/{$container_name}/memory/history?from=$from'"], $server, false);
            if (str($metrics)->contains('error')) {
                $error = json_decode($metrics, true);
                $error = data_get($error, 'error', 'Something is not okay, are you okay?');
                if ($error === 'Unauthorized') {
