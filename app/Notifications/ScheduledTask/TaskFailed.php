...
class TaskFailed extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("FlexDEV: [ACTION REQUIRED] Scheduled task ({$this->task->name}) failed.");
    ...
    {
        $message = "FlexDEV: Scheduled task ({$this->task->name}) failed with output: {$this->output}";
        if ($this->url) {
            $buttons[] = [
                'text' => 'Open task in FlexDEV',
                'url' => (string) $this->url,
            ];
        }
