...
CDN="https://cdn.cloudflex.io/flexdev-nightly"
DATE=$(date +"%Y%m%d-%H%M%S")

OS_TYPE=$(grep -w "ID" /etc/os-release | cut -d "=" -f 2 | tr -d '"')
ENV_FILE="/data/flexdev/source/.env"
DOCKER_VERSION="27.0"
# TODO: Ask for a user
CURRENT_USER=$USER
...
echo -e "Welcome to FlexDEV Installer!"
echo -e "This script will install everything for you. Sit back and relax."
echo -e "Source code: https://github.com/cloudflex/flexdev/blob/v4.x/scripts/install.sh"
...
if [ -f "/data/flexdev/source/.env" ] && [ "$DOCKER_POOL_BASE_PROVIDED" = false ] && [ "$DOCKER_POOL_SIZE_PROVIDED" = false ]; then
    ENV_DOCKER_ADDRESS_POOL_BASE=$(grep -E "^DOCKER_ADDRESS_POOL_BASE=" /data/flexdev/source/.env | cut -d '=' -f2 || true)
    ENV_DOCKER_ADDRESS_POOL_SIZE=$(grep -E "^DOCKER_ADDRESS_POOL_SIZE=" /data/flexdev/source/.env | cut -d '=' -f2 || true)
...
mkdir -p /data/flexdev/{source,ssh,applications,databases,backups,services,proxy,webhooks-during-maintenance,sentinel}
mkdir -p /data/flexdev/ssh/{keys,mux}
mkdir -p /data/flexdev/proxy/dynamic

chown -R 9999:root /data/flexdev
chmod -R 700 /data/flexdev

INSTALLATION_LOG_WITH_DATE="/data/flexdev/source/installation-${DATE}.log"
...
if [ "$SSH_DETECTED" = "false" ]; then
    ...
    *)
        ...
        echo -e "Please make sure it is installed and running, otherwise FlexDEV cannot connect to the host system. \n"
        echo "###############################################################################"
        exit 1
        ;;
    esac
...
if [ "$SSH_PERMIT_ROOT_LOGIN" = "yes" ] || [ "$SSH_PERMIT_ROOT_LOGIN" = "without-password" ] || [ "$SSH_PERMIT_ROOT_LOGIN" = "prohibit-password" ]; then
    ...
    echo "   If you have problems with SSH, please read this: https://flexdev.io/docs/knowledge-base/server/openssh"
fi
...
if [ -x "$(command -v snap)" ]; then
    ...
    if [ "$SNAP_DOCKER_INSTALLED" = "true" ]; then
        ...
        echo "   Please note that FlexDEV does not support Docker installed via snap."
...
# Create flexdev configuration with or without address pools based on whether they were explicitly provided
...
EOL
    ...
    if [ -f /etc/docker/daemon.json ] && jq -e '.["log-driver"] == "json-file" and .["log-opts"]["max-size"] == "10m" and .["log-opts"]["max-file"] == "3"' /etc/docker/daemon.json >/dev/null 2>&1; then
        ...
        cat >/etc/docker/daemon.json.flexdev <<EOL
{
  "log-driver": "json-file",
  "log-opts": {
    "max-size": "10m",
...
curl -fsSL $CDN/docker-compose.yml -o /data/flexdev/source/docker-compose.yml
curl -fsSL $CDN/docker-compose.prod.yml -o /data/flexdev/source/docker-compose.prod.yml
curl -fsSL $CDN/.env.production -o /data/flexdev/source/.env.production
curl -fsSL $CDN/upgrade.sh -o /data/flexdev/source/upgrade.sh
...
if [ -f "$ENV_FILE" ]; then
    ...
    awk -F '=' '!seen[$1]++' "$ENV_FILE" "/data/flexdev/source/.env.production" > "$ENV_FILE.tmp" && mv "$ENV_FILE.tmp" "$ENV_FILE"
    ...
    cp "/data/flexdev/source/.env.production" "$ENV_FILE"
fi
...
set +e
IS_FLEXDEV_VOLUME_EXISTS=$(docker volume ls | grep flexdev-db | wc -l)
set -e

if [ "$IS_FLEXDEV_VOLUME_EXISTS" -eq 0 ]; then
    echo " - Generating SSH key."
    test -f /data/flexdev/ssh/keys/id.$CURRENT_USER@host.docker.internal && rm -f /data/flexdev/ssh/keys/id.$CURRENT_USER@host.docker.internal
    test -f /data/flexdev/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub && rm -f /data/flexdev/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
    ssh-keygen -t ed25519 -a 100 -f /data/flexdev/ssh/keys/id.$CURRENT_USER@host.docker.internal -q -N "" -C flexdev
    chown 9999 /data/flexdev/ssh/keys/id.$CURRENT_USER@host.docker.internal
    sed -i "/flexdev/d" ~/.ssh/authorized_keys
    cat /data/flexdev/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub >>~/.ssh/authorized_keys
    rm -f /data/flexdev/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
fi

chown -R 9999:root /data/flexdev
chmod -R 700 /data/flexdev

echo -e "9. Installing FlexDEV ($LATEST_VERSION)"
echo -e " - It could take a while based on your server's performance, network speed, stars, etc."
echo -e " - Please wait."
getAJoke

if [[ $- == *x* ]]; then
    bash -x /data/flexdev/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-ghcr.io}" "true"
else
    bash /data/flexdev/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-ghcr.io}" "true"
fi
echo " - FlexDEV installed successfully."

echo " - Waiting 20 seconds for FlexDEV database migrations to complete."
getAJoke
...
if [ -n "$IPV4_PUBLIC_IP" ]; then
    echo -e "You can access FlexDEV through your Public IPV4: http://$IPV4_PUBLIC_IP:8000"
fi
if [ -n "$IPV6_PUBLIC_IP" ]; then
    echo -e "You can access FlexDEV through your Public IPv6: http://[$IPV6_PUBLIC_IP]:8000"
fi
...
echo -e "\nWARNING: It is highly recommended to backup your Environment variables file (/data/flexdev/source/.env) to a safe location, outside of this server (e.g. into a Password Manager).\n"
