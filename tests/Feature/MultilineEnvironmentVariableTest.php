...
test('multiline environment variables are properly escaped for docker build args', function () {
    $sshKey = "FAKE_MULTILINE_KEY_LINE_1\nFAKE_MULTILINE_KEY_LINE_2\nFAKE_MULTILINE_KEY_LINE_3\nFAKE_MULTILINE_KEY_LINE_4\nFAKE_MULTILINE_KEY_LINE_5";
    ...
    expect($sshArg)->toContain('FAKE_MULTILINE_KEY_LINE_1');
    expect($sshArg)->not->toContain("'FAKE_MULTILINE_KEY_LINE_1"); // Should not have the wrapper single quotes
...
test('real world SSH key example', function () {
    ...
    expect($arg)->toStartWith('--build-arg KEY="FAKE_MULTILINE_KEY_LINE_1');
    expect($arg)->toEndWith('FAKE_MULTILINE_KEY_LINE_5"');
