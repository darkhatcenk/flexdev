...
it('generates url using base_url instead of APP_URL', function () {
    ...
    ($this->setInstanceSettings)('https://flexdev.example.com');
    ...
    expect($notification->serverUrl)->toBe('https://flexdev.example.com/server/test-server-uuid/security/patches');
});
...
it('includes server url in all notification channels', function () {
    ($this->setInstanceSettings)('https://flexdev.test');
    ...
    expect($discord->description)->toContain('https://flexdev.test/server/abc-123/security/patches');

    // Check Telegram
    $telegram = $notification->toTelegram();
    expect($telegram['buttons'][0]['url'])->toBe('https://flexdev.test/server/abc-123/security/patches');

    // Check Pushover
    $pushover = $notification->toPushover();
    expect($pushover->buttons[0]['url'])->toBe('https://flexdev.test/server/abc-123/security/patches');

    // Check Slack
    $slack = $notification->toSlack();
    expect($slack->description)->toContain('https://flexdev.test/server/abc-123/security/patches');

    // Check Webhook
    $webhook = $notification->toWebhook();
    expect($webhook['url'])->toBe('https://flexdev.test/server/abc-123/security/patches');
});

it('uses correct url in error notifications', function () {
    ($this->setInstanceSettings)('https://flexdev.production.com');
    ...
    $discord = $notification->toDiscord();
    expect($discord->description)->toContain('https://flexdev.production.com/server/error-server-uuid/security/patches');

    // Check error webhook
    $webhook = $notification->toWebhook();
    expect($webhook['url'])->toBe('https://flexdev.production.com/server/error-server-uuid/security/patches')
