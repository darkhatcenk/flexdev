 ...
 */
it('verifies docker_compose_raw does not contain FlexDEV labels after parsing', function () {
...
YAML;
    ...
    // Check that docker_compose_raw does NOT contain FlexDEV labels
    $labels = data_get($yamlAfterParsing, 'services.web.labels', []);
    $hasTraefikLabels = false;
    $hasFlexDEVManagedLabel = false;
    ...
    foreach ($labels as $label) {
        if (is_string($label)) {
            ...
            if (str_contains($label, 'flexdev.managed')) {
                $hasFlexDEVManagedLabel = true;
            }
        }
    }

    // docker_compose_raw should NOT have FlexDEV additions
    expect($hasTraefikLabels)->toBeFalse('docker_compose_raw should not contain Traefik labels');
    expect($hasFlexDEVManagedLabel)->toBeFalse('docker_compose_raw should not contain flexdev.managed label');
