...
it('injects -f and --env-file flags into custom start command', function () {
    ...
    $serverWorkdir = '/var/lib/docker/volumes/flexdev-data/_data/applications/app-uuid';
    ...
    expect($customCommand)->toBe('docker compose -f /var/lib/docker/volumes/flexdev-data/_data/applications/app-uuid/docker-compose.yaml --env-file /var/lib/docker/volumes/flexdev-data/_data/applications/app-uuid/.env up -d');
    expect($customCommand)->toContain('-f /var/lib/docker/volumes/flexdev-data/_data/applications/app-uuid/docker-compose.yaml');
    expect($customCommand)->toContain('--env-file /var/lib/docker/volumes/flexdev-data/_data/applications/app-uuid/.env');
});
...
it('does not duplicate -f flag in start command when already present', function () {
    ...
    expect($customCommand)->toBe('docker compose --env-file /var/lib/docker/volumes/flexdev-data/_data/applications/app-uuid/.env -f ./custom-compose.yaml up -d');
    expect(substr_count($customCommand, ' -f '))->toBe(1);
...
it('does not duplicate --env-file flag in start command when already present', function () {
    ...
    expect($customCommand)->toBe('docker compose -f /var/lib/docker/volumes/flexdev-data/_data/applications/app-uuid/docker-compose.yaml --env-file ./my.env up -d');
    expect(substr_count($customCommand, '--env-file'))->toBe(1);
