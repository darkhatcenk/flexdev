...
it('ensures stop proxy includes wait loop for container removal', function () {
    ...
    $commands = [
        'docker stop --time=30 flexdev-proxy 2>/dev/null || true',
    ...
    expect($commandsString)->toContain('docker stop --time=30 flexdev-proxy')
        ->and($commandsString)->toContain('docker rm -f flexdev-proxy')
        ->and($commandsString)->toContain('for i in {1..10}; do')
        ->and($commandsString)->toContain('if ! docker ps -a --format "{{.Names}}" | grep -q "^flexdev-proxy$"')
...
it('uses configurable timeout for docker stop', function () {
    ...
    $timeout = 30;
    $stopCommand = "docker stop --time=$timeout flexdev-proxy 2>/dev/null || true";
...
it('waits for swarm service container removal correctly', function () {
    ...
    $containerName = 'flexdev-proxy_traefik';
    $checkCommand = "    if ! docker ps -a --format \"{{.Names}}\" | grep -q \"^$containerName$\"; then";

    expect($checkCommand)->toContain('flexdev-proxy_traefik');
});
