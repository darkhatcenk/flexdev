...
it('ensures container cleanup includes wait loop in command sequence', function () {
    ...
    $commands = collect([
        ...
        'cd /data/flexdev/proxy',
        "echo 'Creating required Docker Compose file.'",
        "echo 'Pulling docker image.'",
        'docker compose pull',
        'if docker ps -a --format "{{.Names}}" | grep -q "^flexdev-proxy$"; then',
        "    echo 'Stopping and removing existing flexdev-proxy.'",
        '    docker stop flexdev-proxy 2>/dev/null || true',
        '    docker rm -f flexdev-proxy 2>/dev/null || true',
        '    # Wait for container to be fully removed',
        '    for i in {1..10}; do',
        '        if ! docker ps -a --format "{{.Names}}" | grep -q "^flexdev-proxy$"; then',
        '            break',
        '        fi',
        '        echo "Waiting for flexdev-proxy to be removed... ($i/10)"',
        '        sleep 1',
        '    done',
        "    echo 'Successfully stopped and removed existing flexdev-proxy.'",
        'fi',
        "echo 'Starting flexdev-proxy.'",
        'docker compose up -d --wait --remove-orphans',
        "echo 'Successfully started flexdev-proxy.'",
    ]);

    $commandsString = $commands->implode("\n");

    // Verify the cleanup sequence includes all required components
    expect($commandsString)->toContain('docker stop flexdev-proxy 2>/dev/null || true')
        ->and($commandsString)->toContain('docker rm -f flexdev-proxy 2>/dev/null || true')
        ->and($commandsString)->toContain('for i in {1..10}; do')
        ->and($commandsString)->toContain('if ! docker ps -a --format "{{.Names}}" | grep -q "^flexdev-proxy$"; then')
    ...
    $stopPosition = strpos($commandsString, 'docker stop flexdev-proxy');
