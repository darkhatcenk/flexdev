...
test('adds ownership changes for FlexDEV data paths', function () {
    $commands = collect([
        'mkdir -p /data/flexdev/logs',
    ]);
    ...
    expect($result[0])->toBe('sudo mkdir -p /data/flexdev/logs && sudo sudo chown -R ubuntu:ubuntu /data/flexdev/logs && sudo sudo chmod -R o-rwx /data/flexdev/logs');
});

test('adds ownership changes for FlexDEV tmp paths', function () {
    $commands = collect([
        'mkdir -p /tmp/flexdev/cache',
    ]);
    ...
    // Note: The && operator adds another sudo, creating double sudo for chown/chmod
    // This is existing behavior that may need refactoring but isn't part of this bug fix
    expect($result[0])->toBe('sudo mkdir -p /tmp/flexdev/cache && sudo sudo chown -R ubuntu:ubuntu /tmp/flexdev/cache && sudo sudo chmod -R o-rwx /tmp/flexdev/cache');
});
