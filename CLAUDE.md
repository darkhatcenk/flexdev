...
FlexDEV is an open-source, self-hostable platform for deploying applications and managing servers - an alternative to Heroku/Netlify/Vercel. It's built with Laravel (PHP) and uses Docker for containerization.
...
### Backend Development
Only run artisan commands inside "flexdev" container when in development.
...
- **Inside Docker**: `docker exec flexdev php artisan test` (for feature tests requiring database)
...
- Feature tests that require database must be run in the `flexdev` container
...
- `Server` - Remote servers managed by FlexDEV (46KB, complex)
...
### Frontend Philosophy
FlexDEV uses a **server-side first** approach with minimal JavaScript:
...
- **Feature Tests** (`tests/Feature/`): May use database. MUST run inside Docker with `docker exec flexdev php artisan test`
...
We have a cloud instance of FlexDEV (hosted version) with:
...
- [Project Overview](.ai/core/project-overview.md) - What FlexDEV is and how it works
...
- **Feature tests**: Can use database, run with `docker exec flexdev php artisan test`
