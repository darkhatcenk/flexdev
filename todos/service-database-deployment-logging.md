...
4. **Existing Pattern:**
   - FlexDEV already uses separate tables: `applications`, `services`, `standalone_*`
...
**File:** `app/Actions/FlexDEVTask/PrepareFlexDEVTask.php`
...
**Note:** Exact implementation depends on PrepareFlexDEVTask structure. Need to review file in detail during implementation.
...
### Feature Tests (Run inside Docker: `docker exec flexdev php artisan test`)
...
**Details:** `PrepareFlexDEVTask` is core to all deployments. Changes here affect everything.

**Mitigation:**
- Review `PrepareFlexDEVTask` code in detail before changes
...
- [ ] Review `PrepareFlexDEVTask` code
