                    ...
                    @if (!isCloud())
                        <div class="w-48">
                            <x-forms.checkbox canGate="update" :canResource="$github_app" label="System Wide?"
                                helper="If checked, this GitHub App will be available for everyone in this FlexDEV instance."
                                instantSave id="isSystemWide" />
                        </div>
                        @if ($isSystemWide)
                            <x-callout type="warning" title="Not Recommended">
                                System-wide GitHub Apps are shared across all teams on this FlexDEV instance. This means any team can use this GitHub App to deploy applications from your repositories. For better security and isolation, it's recommended to create team-specific GitHub Apps instead.
                            </x-callout>
                        @endif
                    @endif
                    <div class="flex flex-col sm:flex-row gap-2">
                        ...
                        @if (!isCloud() || isDev())
                            <div class="flex flex-col sm:flex-row items-start sm:items-end gap-2">
                                <x-forms.select wire:model.live='webhook_endpoint' label="Webhook Endpoint"
                                    helper="All Git webhooks will be sent to this endpoint. <br><br>If you would like to use domain instead of IP address, set your FlexDEV instance's FQDN in the Settings menu.">
                                    @if ($ipv4)
