    ...
    @if ($this->deploymentCount > 0)
        <div class="relative">
            ...
            <button @click="expanded = !expanded"
                ...
                <!-- Animated spinner -->
                <svg class="w-4 h-4 text-cloudflex dark:text-warning animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 24 24">
                ...
                <div class="p-4 space-y-3">
                    @foreach ($this->deployments as $deployment)
                        <a href="{{ $deployment->deployment_url }}"
                            class="flex items-start gap-3 p-3 rounded-lg dark:bg-coolgray-200 bg-gray-50 transition-all duration-200 hover:ring-2 hover:ring-cloudflex dark:hover:ring-warning cursor-pointer">
                            <!-- Status indicator -->
                            <div class="flex-shrink-0 mt-1">
                                @if ($deployment->status === 'in_progress')
                                    <svg class="w-4 h-4 text-cloudflex dark:text-warning animate-spin"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs mt-1 capitalize"
                                    :class="{
                                        'text-cloudflex dark:text-warning': '{{ $deployment->status }}' === 'in_progress',
