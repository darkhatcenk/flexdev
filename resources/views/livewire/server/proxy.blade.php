            ...
            @if ($selectedProxy !== 'NONE')
                <form wire:submit='submit'>
                        ...
                        @if ($server->proxy->status === 'exited' || $server->proxy->status === 'removing')
                            @can('update', $server)
                                <x-modal-confirmation title="Confirm Proxy Switching?" buttonTitle="Switch Proxy"
                                    ...
                                    warningMessage="This operation may cause issues. Please refer to the guide <a href='https://flexdev.io/docs/knowledge-base/server/proxies#switch-between-proxies' target='_blank' class='underline text-white'>switching between proxies</a> before proceeding!"
                    ...
                    <div class="pb-6 w-96">
                        ...
                        @if ($redirectEnabled)
                            <x-forms.input canGate="update" :canResource="$server" placeholder="https://app.flexdev.io"
