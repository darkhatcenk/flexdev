<div x-data x-init="@if ($server->hetzner_server_id && $server->cloudProviderToken && !$hetznerServerStatus) $wire.checkHetznerServerStatus() @endif">
    <x-slot:title>
        {{ data_get_str($server, 'name')->limit(10) }} > General | FlexDEV
    </x-slot>
        ...
        <div class="w-full">
            <form wire:submit.prevent='submit' class="flex flex-col">
                                ...
                                @if ($hetznerServerStatus)
                                    <span class="pl-1.5">
                                        @if (in_array($hetznerServerStatus, ['starting', 'initializing']))
                                            <svg class="inline animate-spin h-3 w-3 mr-1 text-cloudflex dark:text-yellow-500"
                    ...
                    @if ($server->id === 0)
                        <x-modal-confirmation title="Confirm Server Settings Change?" buttonTitle="Save"
                            submitAction="submit" :actions="[
                                'If you misconfigure the server, you could lose a lot of functionalities of FlexDEV.',
                ...
                @if (
                    ...
                    <x-slide-over closeWithX fullScreen>
                        ...
                        <x-forms.button @click="slideOverOpen=true"
                            class="mt-8 mb-4 w-full font-bold box-without-bg bg-cloudflex hover:bg-cloudflex-100"
                ...
                @if ((!$isReachable || !$isUsable) && $server->id === 0)
                    <x-forms.button class="mt-8 mb-4 font-bold box-without-bg bg-cloudflex hover:bg-cloudflex-100"
                        ...
                        @if (!$server->isLocalhost())
                            ...
                            @if (!$server->isBuildServer() && !$server->settings->is_cloudflare_tunnel)
                                ...
                                <div class="pb-4">Read the docs <a class='underline dark:text-white'
                                        href='https://flexdev.io/docs/knowledge-base/docker/swarm'
                                        target='_blank'>here</a>.
                                </div>
                                <div class="w-96">
                                    @if ($server->settings->is_swarm_worker)
                                        <x-forms.checkbox disabled instantSave type="checkbox" id="isSwarmManager"
                                            helper="For more information, please read the documentation <a class='dark:text-white' href='https://flexdev.io/docs/knowledge-base/docker/swarm' target='_blank'>here</a>."
                                    ...
                                    <x-slide-over fullScreen>
                                        ...
                                        <x-slot:content>
                                            <livewire:project.shared.get-logs :server="$server"
                                                container="flexdev-sentinel" displayName="Sentinel" lazy />
                                        </x-slot:content>
                                        <x-forms.button @click="slideOverOpen=true"
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        @if ($server->isSentinelEnabled())
                            ...
                            <x-forms.input canGate="update" :canResource="$server" id="sentinelCustomUrl" required
                                label="FlexDEV URL"
                                helper="URL to your FlexDEV instance. If it is empty that means you do not have a FQDN set for your FlexDEV instance."
                                :disabled="$isValidating" />
