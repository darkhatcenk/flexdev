        ...
        <div class="flex flex-col gap-2">
            @if($requiredPort && !$application->serviceType()?->contains(str($application->image)->before(':')))
                <x-callout type="warning" title="Required Port: {{ $requiredPort }}" class="mb-2">
                    ...
                    <br><br>
                    <strong>Example:</strong> http://app.flexdev.io:{{ $requiredPort }}
                </x-callout>
            @endif
            ...
            <div class="flex gap-2">
                @if (!$application->serviceType()?->contains(str($application->image)->before(':')))
                    @if ($application->required_fqdn)
                        <x-forms.input canGate="update" :canResource="$application" required placeholder="https://app.flexdev.io"
                    ...
                    @else
                        <x-forms.input canGate="update" :canResource="$application" placeholder="https://app.flexdev.io"
                            label="Domains" id="fqdn"
                            helper="You can specify one domain with path or more with comma. You can specify a port to bind the domain to.<br><br><span class='text-helper'>Example</span><br>- http://app.flexdev.io,https://cloud.flexdev.io/dashboard<br>- http://app.flexdev.io/api/v3<br>- http://app.flexdev.io:3000 -> app.flexdev.io will point to port 3000 inside the container. "></x-forms.input>
                    @endif
                @endif
