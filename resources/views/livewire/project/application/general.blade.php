    ...
    <form wire:submit='submit' class="flex flex-col pb-32">
        ...
        <div class="flex flex-col gap-2 py-4">
                            ...
                            @foreach (data_get($parsedServices, 'services') as $serviceName => $service)
                                @if (!isDatabaseImage(data_get($service, 'image')))
                                    <div class="flex items-end gap-2">
                                        <x-forms.input
                                            helper="You can specify one domain with path or more with comma. You can specify a port to bind the domain to.<br><br><span class='text-helper'>Example</span><br>- http://app.flexdev.io,https://cloud.flexdev.io/dashboard<br>- http://app.flexdev.io/api/v3<br>- http://app.flexdev.io:3000 -> app.flexdev.io will point to port 3000 inside the container. "
            ...
            @if ($application->build_pack !== 'dockercompose')
                <div class="flex items-end gap-2">
                    @if ($application->settings->is_container_label_readonly_enabled == false)
                        <x-forms.input placeholder="https://flexdev.io" wire:model="fqdn" label="Domains" readonly
                            helper="Readonly labels are disabled. You can set the domains in the labels section."
                            x-bind:disabled="!canUpdate" />
                    @else
                        <x-forms.input placeholder="https://flexdev.io" wire:model="fqdn" label="Domains"
                            helper="You can specify one domain with path or more with comma. You can specify a port to bind the domain to.<br><br><span class='text-helper'>Example</span><br>- http://app.flexdev.io,https://cloud.flexdev.io/dashboard<br>- http://app.flexdev.io/api/v3<br>- http://app.flexdev.io:3000 -> app.flexdev.io will point to port 3000 inside the container. "
                            x-bind:disabled="!canUpdate" />
                        @can('update', $application)
            ...
            @if ($application->build_pack !== 'dockercompose')
                <div class="flex items-center gap-2 pt-8">
                    ...
                    @if ($application->build_pack !== 'dockerimage' && !$application->destination->server->isSwarm())
                        <x-helper
                            helper="Push the built image to a docker registry. More info <a class='underline' href='https://flexdev.io/docs/knowledge-base/docker/registry' target='_blank'>here</a>." />
                    @endif
                </div>
                @if ($application->destination->server->isSwarm())
                    @if ($application->build_pack !== 'dockerimage')
                        <div>Docker Swarm requires the image to be available in a registry. More info <a class="underline"
                                href="https://flexdev.io/docs/knowledge-base/docker/registry" target="_blank">here</a>.</div>
                    @endif
                @endif
            ...
            <div>
                ...
                @if ($application->build_pack === 'dockerimage')
                    <x-forms.input
                        helper="You can add custom docker run options that will be used when your container is started.<br>Note: Not all options are supported, as they could mess up FlexDEV's automation and could cause bad experience for users.<br><br>Check the <a class='underline dark:text-white' href='https://flexdev.io/docs/knowledge-base/docker/custom-commands'>docs.</a>"
                    ...
                    @if ($application->could_set_build_commands())
                        @if ($application->build_pack === 'nixpacks')
                            ...
                            <div class="pt-1 text-xs">Nixpacks will detect the required configuration
                                automatically.
                                <a class="underline" href="https://flexdev.io/docs/applications/">Framework
                                    Specific Docs</a>
                            </div>
                        @endif
                    ...
                    <div class="flex flex-col gap-2 pt-6 pb-10">
                        ...
                        @else
                                ...
                                @if ($application->build_pack !== 'dockercompose')
                                    <div class="pt-2 w-96">
                                        <x-forms.checkbox
                                            helper="Use a build server to build your application. You can configure your build server in the Server settings. For more info, check the <a href='https://flexdev.io/docs/knowledge-base/server/build-server' class='underline' target='_blank'>documentation</a>."
                ...
                @if ($application->build_pack === 'dockercompose')
                    ...
                    <div class="w-96">
                        ...
                        {{-- <x-forms.checkbox label="Readonly labels"
                            helper="Labels are readonly by default. Readonly means that edits you do to the labels could be lost and FlexDEV will autogenerate the labels for you. If you want to edit the labels directly, disable this option. <br><br>Be careful, it could break the proxy configuration after you restart the container as FlexDEV will now NOT autogenerate the labels for you (ofc you can always reset the labels to the flexdev defaults manually)."
                ...
                @if ($application->build_pack !== 'dockercompose')
                    ...
                    @can('update', $application)
                            <x-modal-confirmation title="Confirm Labels Reset to FlexDEV Defaults?"
                            ...
                            'Proxy labels (traefik, caddy, etc) will be reset to the flexdev defaults.',
                        ]" confirmationText="{{ $application->fqdn . '/' }}"
