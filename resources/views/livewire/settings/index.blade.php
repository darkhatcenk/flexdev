<div>
    ...
    <div x-data="{ activeTab: window.location.hash ? window.location.hash.substring(1) : 'general' }" class="flex flex-col h-full gap-8 sm:flex-row">
        ...
        <form wire:submit='submit' class="flex flex-col">
            ...
            <div class="pb-4">General configuration for your FlexDEV instance.</div>
                ...
                <div class="flex flex-wrap items-end gap-2">
                    <div class="flex gap-2 md:flex-row flex-col w-full">
                        <x-forms.input id="fqdn" label="Domain"
                            ...
                            placeholder="https://flexdev.yourdomain.com" />
                        <x-forms.input id="instance_name" label="Name" placeholder="FlexDEV"
                            helper="Custom name for your FlexDEV instance, shown in the URL." />
                        <div class="w-full" x-data="{
                            ...
                            <div class="flex items-center mb-1">
                                <label for="instance_timezone">Instance
                                    Timezone</label>
                                <x-helper class="ml-2"
                                    helper="Timezone for the FlexDEV instance. This is used for the update check and automatic update frequency." />
                            </div>
                            <div class="relative">
                    ...
                    <div class="flex gap-2 md:flex-row flex-col w-full">
                        <x-forms.input id="public_ipv4" type="password" label="Instance's Public IPv4"
                            helper="Enter the IPv4 address of the instance.<br><br>It is useful if you have several IPv4 addresses and FlexDEV could not detect the correct one."
                            placeholder="1.2.3.4" autocomplete="new-password" />
                        <x-forms.input id="public_ipv6" type="password" label="Instance's Public IPv6"
                            helper="Enter the IPv6 address of the instance.<br><br>It is useful if you have several IPv6 addresses and FlexDEV could not detect the correct one."
                    ...
                    <div class="flex gap-2 md:flex-row flex-col w-full">
                        <x-forms.input id="dev_helper_version" label="Dev Helper Version (Development Only)"
                            helper="Override the default flexdev-helper image version. Leave empty to use the default version from config ({{ config('constants.flexdev.helper_version') }}). Examples: 1.0.11, latest, dev"
                            placeholder="{{ config('constants.flexdev.helper_version') }}" />
                    </div>
                    @endif
                </div>
            </div>
        ...
        <x-domain-conflict-modal 
            ...
            <x-slot:consequences>
                <ul class="mt-2 ml-4 list-disc">
                    <li>The FlexDEV instance domain will conflict with existing resources</li>
                    <li>SSL certificates might not work correctly</li>
                    <li>Routing behavior will be unpredictable</li>
                    <li>You may not be able to access the FlexDEV dashboard properly</li>
                </ul>
            </x-slot:consequences>
        </x-domain-conflict-modal>
    </div>
