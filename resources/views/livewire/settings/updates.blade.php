            ...
            <div class="flex flex-col gap-2">
                <div class="flex items-end gap-2">
                    <x-forms.input required id="update_check_frequency" label="Update Check Frequency"
                        placeholder="0 * * * *"
                        helper="Frequency (cron expression) to check for new FlexDEV versions and pull new Service Templates from CDN.<br>You can use every_minute, hourly, daily, weekly, monthly, yearly.<br><br>Default is every hour." />
                    ...
                    @if (!is_null(config('constants.flexdev.autoupdate', null)))
                        <div class="text-right">
                            <x-forms.checkbox instantSave
                                ...
                                checked="{{ config('constants.flexdev.autoupdate') }}" label="Enabled" />
                        </div>
                    @else
                        <x-forms.checkbox instantSave id="is_auto_update_enabled" label="Enabled" />
                    @endif
                </div>
                @if (is_null(config('constants.flexdev.autoupdate', null)) && $is_auto_update_enabled)
                    <x-forms.input required id="auto_update_frequency" label="Frequency (cron expression)"
                        placeholder="0 0 * * *"
                        helper="Frequency (cron expression) (automatically update flexdev).<br>You can use every_minute, hourly, daily, weekly, monthly, yearly.<br><br>Default is every day at 00:00" />
                @else
