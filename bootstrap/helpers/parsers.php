...
function applicationParser(Application $resource, int $pull_request_id = 0, ?int $preview_id = null): Collection
{
    ...
    foreach ($services as $serviceName => $service) {
        ...
        // Add FlexDEV defined environments
        ...
        if ($magicEnvironments->count() > 0) {
            // Generate FlexDEV environment variables
    ...
    foreach ($services as $serviceName => $service) {
            ...
            foreach ($volumes as $index => $volume) {
                ...
                if ($type->value() === 'bind') {
                    if ($source->value() === '/var/run/docker.sock') {
                        ...
                        if (isDev()) {
                            if ((int) $resource->compose_parsing_version >= 4) {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/flexdev_dev_flexdev_data/_data/applications/'.$uuid);
                            } else {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/flexdev_dev_flexdev_data/_data/applications/'.$uuid);
                            }
                        }
            ...
            if ($key->value() === $parsedValue->value()) {
                ...
                if ($value->startsWith('$')) {
                    ...
                    if ($originalValue->value() === $value->value()) {
                        // This means the variable does not have a default value, so it needs to be created in FlexDEV
        ...
        if ($originalResource->environment_variables->where('key', 'FLEXDEV_BRANCH')->isEmpty()) {
            $flexdevEnvironments->put('FLEXDEV_BRANCH', "\"{$branch}\"");
        }

        // Add FLEXDEV_RESOURCE_UUID to environment
        if ($resource->environment_variables->where('key', 'FLEXDEV_RESOURCE_UUID')->isEmpty()) {
            $flexdevEnvironments->put('FLEXDEV_RESOURCE_UUID', "{$resource->uuid}");
        }

        // Add FLEXDEV_CONTAINER_NAME to environment
        if ($resource->environment_variables->where('key', 'FLEXDEV_CONTAINER_NAME')->isEmpty()) {
            $flexdevEnvironments->put('FLEXDEV_CONTAINER_NAME', "{$containerName}");
        }
        ...
        if ($resource->build_pack === 'dockercompose') {
            foreach ($domains as $forServiceName => $domain) {
                ...
                if (filled($parsedDomain)) {
                    ...
                    $flexdevEnvironments->put('SERVICE_URL_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'), $flexdevUrl->__toString());
                    $flexdevEnvironments->put('SERVICE_FQDN_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'), $flexdevFqdn);
                    $resource->environment_variables()->updateOrCreate([
                        'resourceable_type' => Application::class,
                        'resourceable_id' => $resource->id,
                        'key' => 'SERVICE_URL_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'),
                    ], [
                        'value' => $flexdevUrl->__toString(),
                        'is_preview' => false,
                    ]);
                    $resource->environment_variables()->updateOrCreate([
                        'resourceable_type' => Application::class,
        ...
        $isDatabase = isDatabaseImage($image, $service);
        // Add FLEXDEV_FQDN & FLEXDEV_URL to environment
        if (! $isDatabase && $fqdns instanceof Collection && $fqdns->count() > 0) {
            $fqdnsWithoutPort = $fqdns->map(function ($fqdn) {
                return str($fqdn)->after('://')->before(':')->prepend(str($fqdn)->before('://')->append('://'));
            });
            $flexdevEnvironments->put('FLEXDEV_URL', $fqdnsWithoutPort->implode(','));

            $urls = $fqdns->map(function ($fqdn) {
                return str($fqdn)->replace('http://', '')->replace('https://', '')->before(':');
            });
            $flexdevEnvironments->put('FLEXDEV_FQDN', $urls->implode(','));
        }
        add_flexdev_default_environment_variables($resource, $flexdevEnvironments, $resource->environment_variables);
        ...
        if ($environment->count() > 0 || $flexdevEnvironments->count() > 0) {
            $payload['environment'] = $environment->merge($flexdevEnvironments)->merge($serviceNameEnvironments);
        }
        if ($logging) {
        ...
        // Auto-inject .env file so FlexDEV environment variables are available inside containers
    ...
    // Parse the original compose again to create a clean version without FlexDEV additions
    try {
...
function serviceParser(Service $resource): Collection
{
        ...
        foreach ($environment as $key => $value) {
            ...
            if ($key->startsWith('SERVICE_FQDN_') || $key->startsWith('SERVICE_URL_')) {
                ...
                if ($isServiceApplication && is_null($savedService->fqdn)) {
                    ...
                    if ((int) $resource->compose_parsing_version >= 5 && version_compare(config('constants.flexdev.version'), '4.0.0-beta.420.7', '>=')) {
    ...
    foreach ($services as $serviceName => $service) {
            ...
            foreach ($volumes as $index => $volume) {
                ...
                if ($type->value() === 'bind') {
                    if ($source->value() === '/var/run/docker.sock') {
                        ...
                        if (isDev()) {
                            if ((int) $resource->compose_parsing_version >= 4) {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/flexdev_dev_flexdev_data/_data/services/'.$uuid);
                            } else {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/flexdev_dev_flexdev_data/_data/applications/'.$uuid);
                            }
                        }
        ...
        if (! $isDatabase && $fqdns instanceof Collection && $fqdns->count() > 0) {
            ...
            $flexdevEnvironments->put('FLEXDEV_FQDN', $fqdnsWithoutPort->implode(','));
            $urls = $fqdns->map(function ($fqdn): Stringable {
                return str($fqdn)->after('://')->before(':')->prepend(str($fqdn)->before('://')->append('://'));
            });
            $flexdevEnvironments->put('FLEXDEV_URL', $urls->implode(','));
        }
        add_flexdev_default_environment_variables($resource, $flexdevEnvironments, $resource->environment_variables);
        if ($environment->count() > 0) {
            $environment = $environment->filter(function ($value, $key) {
