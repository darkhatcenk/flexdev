...
function getCurrentApplicationContainerStatus(Server $server, int $id, ?int $pullRequestId = null, ?bool $includePullrequests = false): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=flexdev.applicationId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);

        $containers = $containers->map(function ($container) use ($pullRequestId, $includePullrequests) {
            $labels = data_get($container, 'Labels');
            $containerName = data_get($container, 'Names');
            $hasPrLabel = str($labels)->contains('flexdev.pullRequestId=');
            $prLabelValue = null;

            if ($hasPrLabel) {
                preg_match('/flexdev\.pullRequestId=(\d+)/', $labels, $matches);
            ...
            if ($pullRequestId !== null && $pullRequestId !== 0 && str($labels)->contains("flexdev.pullRequestId={$pullRequestId}")) {
                return $container;
            }
...
function getCurrentServiceContainerStatus(Server $server, int $id): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=flexdev.serviceId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);
...
{
    $labels = collect([]);
    $labels->push('flexdev.managed=true');
    $labels->push('flexdev.type=database');
    $labels->push('flexdev.databaseId='.$database->id);
    $labels->push('flexdev.resourceName='.Str::slug($database->name));
    $labels->push('flexdev.serviceName='.Str::slug($database->name));
    $labels->push('flexdev.projectName='.Str::slug($database->project()->name));
    $labels->push('flexdev.environmentName='.Str::slug($database->environment->name));
    $labels->push('flexdev.database.subType='.$database->type());
...
function defaultLabels($id, $name, string $projectName, string $resourceName, string $environment, $pull_request_id = 0, string $type = 'application', $subType = null, $subId = null, $subName = null)
{
    $labels = collect([]);
    $labels->push('flexdev.managed=true');
    $labels->push('flexdev.version='.config('constants.flexdev.version'));
    $labels->push('flexdev.'.$type.'Id='.$id);
    $labels->push("flexdev.type=$type");
    $labels->push('flexdev.name='.$name);
    $labels->push('flexdev.resourceName='.Str::slug($resourceName));
    $labels->push('flexdev.projectName='.Str::slug($projectName));
    $labels->push('flexdev.serviceName='.Str::slug($subName ?? $resourceName));
    $labels->push('flexdev.environmentName='.Str::slug($environment));

    $labels->push('flexdev.pullRequestId='.$pull_request_id);
    if ($type === 'service') {
        $subId && $labels->push('flexdev.service.subId='.$subId);
        $subType && $labels->push('flexdev.service.subType='.$subType);
        $subName && $labels->push('flexdev.service.subName='.Str::slug($subName));
    }
...
function fqdnLabelsForTraefik(string $uuid, Collection $domains, bool $is_force_https_enabled = false, $onlyPort = null, ?Collection $serviceLabels = null, ?bool $is_gzip_enabled = true, ?bool $is_stripprefix_enabled = true, ?string $service_name = null, bool $generate_unique_uuid = false, ?string $image = null, string $redirect_direction = 'both', bool $is_http_basic_auth_enabled = false, ?string $http_basic_auth_username = null, ?string $http_basic_auth_password = null)
{
    ...
    if ($serviceLabels) {
        $middlewares_from_labels = $serviceLabels->map(function ($item) {
            ...
            if (preg_match('/flexdev\.traefik\.middlewares=(.*)/', $item, $matches)) {
...
/**
 * Remove FlexDEV's custom Docker Compose fields from parsed YAML array
 *
 * FlexDEV extends Docker Compose with custom fields that are processed during
 ...
 */
function stripFlexDEVCustomFields(array $yamlCompose): array
{
...
function validateComposeFile(string $compose, int $server_id): string|Throwable
{
    ...
    try {
        ...
        // Remove FlexDEV's custom fields before Docker validation
        $yaml_compose = stripFlexDEVCustomFields($yaml_compose);
