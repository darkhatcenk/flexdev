# FlexDEV Release Guide

This guide outlines the release process for FlexDEV, intended for developers and those interested in understanding how FlexDEV releases are managed and deployed.
...
5. **Updating the CDN**
   - To make a new version publicly available, the version information on the CDN needs to be updated manually. After that the new version number will be available at [https://cdn.cloudflex.io/flexdev/versions.json](https://cdn.cloudflex.io/flexdev/versions.json).
...
- **Nightly**
  ...
  - **Installation Command:**
    ```bash
    curl -fsSL https://cdn.cloudflex.io/flexdev-nightly/install.sh | bash -s next
    ```
...
> [!IMPORTANT]
> The cloud version of FlexDEV may be several versions behind the latest GitHub releases even if the CDN is updated. This is intentional to ensure stability and reliability for cloud users and Andras will manully update the cloud version when the update is ready.
...
To update your FlexDEV instance to a specific version, use the following command:

```bash
curl -fsSL https://cdn.cloudflex.io/flexdev/install.sh | bash -s <version>
```
