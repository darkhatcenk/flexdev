# FlexDEV Deployment Architecture

## Deployment Philosophy

FlexDEV orchestrates **Docker-based deployments** across multiple servers with automated configuration generation, zero-downtime deployments, and comprehensive monitoring.
...
```
Git Repository → Webhook → FlexDEV → Build & Deploy
```
...
## FlexDEV Docker Compose Extensions

FlexDEV extends standard Docker Compose with custom fields (often called "magic fields") that provide FlexDEV-specific functionality. These extensions are processed during deployment and stripped before sending the final compose file to Docker, maintaining full compatibility with Docker's compose specification.

### Overview

**Why Custom Fields?**
- Enable FlexDEV-specific features without breaking Docker Compose compatibility
...
2. FlexDEV parses and processes custom fields (creates files, stores settings)
...
- Validation: `tests/Unit/StripFlexDEVCustomFieldsTest.php`
...
**Function:** `stripFlexDEVCustomFields()` in `bootstrap/helpers/docker.php`
...
$yaml_compose = stripFlexDEVCustomFields($yaml_compose);  // Strip custom fields
...
Custom fields are **FlexDEV-specific** and won't work with standalone `docker compose` CLI:

```bash
# ❌ Won't work - Docker doesn't recognize custom fields
docker compose -f compose.yaml up

# ✅ Works - Use FlexDEV's deployment (strips custom fields first)
# Deploy through FlexDEV UI or API
```
...
**Unit Tests:**
- `tests/Unit/StripFlexDEVCustomFieldsTest.php` - Custom field stripping logic
